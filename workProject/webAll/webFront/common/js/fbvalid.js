...
//              }
    ...
    sendValidCode:function(phonenum,param,set){
    	if(param!="")
		{
    		...
    		var imgVerifyCode = $("#" + params[0]).val();
    		var permanentId = $("#" + params[1]).val();
    		if(imgVerifyCode==""||permanentId=="")
			{
    			FBValid.ajax_valid_fail("图形验证码不正确，请刷新后重试",set);
	            phoneMsg.innerHTML="";
	            return false;
			}else{
				$.post("/sendVerifyCodeWithNoForward",{'mobileNum':phonenum,'imgVerifyCode':imgVerifyCode,'permanentId':permanentId},function(data, type) {
			         if (data.responseText == 1) {
			              	FBValid.ajax_valid_access(set);
			...
			$('.imgcode-tip1').hide();
            $('.imgcode-tip2').hide();
            $('.imgcode-tip3').css("display","inline-block");
            $("#imgVerifyCode").val("");
            return false;
		} 
    },
    sendVValidCode:function(phonenum,param,set){
    	if(param!="")
		{
    		var params = param[0].split(",");
    		var imgVerifyCode = $("#" + params[0]).val();
    		...
    		if(imgVerifyCode==""||permanentId=="")
			{
				...
				$.post("/sendVoiceVerifyCode",{'mobileNum':phonenum,'imgVerifyCode':imgVerifyCode,'permanentId':permanentId},function(data, type) {
			         if (data.responseText == 1) {
			              FBValid.ajax_valid_access(set);
			            }else if(data.responseText == 2){
			                FBValid.ajax_valid_fail("手机号码已经注册",set);
			             ...
			             phoneMsg.innerHTML="";
			                ...
			                FBValid.ajax_valid_fail("语音验证码发送失败，请稍后再试",set);
			                return false;
			            }
			        })
			}
    ...
    checkVerifyCode:function(imgVerifyCode,param,set){
    	if(imgVerifyCode!="")
    	{
				if(param!="")
				{
		    		var params = param[0].split(",");
		    		var phonenum = $("#" + params[0]).val();
		    		var permanentId = $("#" + params[1]).val();
		    		$.post("/register/singleCheck",{'imgVerifyCode':imgVerifyCode,'phone':phonenum,'cookieValue':permanentId},function(data) {
		    			    data = eval("(" + data + ")");
    ...
    sendMobileForPassword:function(phonenum,param,set){
    	var params = param[0].split(",");
		var pute = parseInt($("#" + params[0]).val());
        $.post("/password/sendMobileForUpdatePassword",{'phone':phonenum,'pute':pute},function(data, type) {
        	if(data!=''){
        		FBValid.ajax_valid_fail("验证短信发送失败，请稍后再试",set);
        		return false;
        	}else{
    ...
    sendVoiceMobileForPassword:function(phonenum,param,set){
    	var params = param[0].split(",");
		var pute = parseInt($("#" + params[0]).val());
        $.post("/password/sendVoiceMobileForUpdatePassword",{'phone':phonenum,'pute':pute},function(data, type) {
        	if(data!=''){
        		FBValid.ajax_valid_fail("验证短信发送失败，请稍后再试",set);
        		return false;
        	}else{
